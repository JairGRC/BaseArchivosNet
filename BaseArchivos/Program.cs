        ...
        public static void EDomain(string EstructuraBase, string proyect, string fileClase, string RutaDomain)
        {
            ...
            string[] DomainBase =
            {
                ...
                EstructuraBase,
                 ...
                 string.Format(" public async Task<bool> Create{0}({0}Entity {0})",fileClase),
                 "{\n throw new NotImplementedException(); ",
                "//     long id = 0;",
                "//bool exito = false;",
                ...
                "//return id > 0;",
                "\n}",
                 string.Format(" public async Task<bool> Edit{0}({0}Entity {0})",fileClase),
                 "{\n throw new NotImplementedException(); ",
                "//     using (TransactionScope tx = new TransactionScope())",
                "//{",
                ...
                "//return false;",
                "\n}",
                 string.Format(" public async Task<bool> Delete{0}(string cPerCodigo)",fileClase),
                 "{\n throw new NotImplementedException();",
                    "// bool exito = false;",
                    "//exito = _PernameRepository.delete(nConstcodigo, dPerFecEfectiva);",
                    "//return exito;",
                " \n}",
                 string.Format("public async Task<{0}Entity> GetByItem({0}Filter filter, {0}FilterItemType filterType)",fileClase),
                 "{",
                 string.Format("{0}Entity {0} = null;",fileClase),
                 string.Format("{0} = await _{0}Repository.GetItem(filter,filterType);",fileClase),
                 ...
                 "}",
                 string.Format("public async Task<IEnumerable<{0}Entity>> GetByList({0}Filter filter,{0}FilterListType filterType, Pagination pagination)",fileClase),
                 "{",
                 string.Format("IEnumerable<{0}Entity> lst = null;",fileClase),
                 string.Format("lst =await  _{0}Repository.GetLstItem(filter, filterType, pagination);",fileClase),
                 "return lst;",
                 "}",
                 "#endregion",
                 "}",
